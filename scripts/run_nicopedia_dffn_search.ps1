...
function Invoke-ResearchSegment($Candidate, [int]$Seed, [int]$Steps, [string]$Stage, [int]$ResumeStep = 0, [switch]$FullCapEval) {
    ...
    $script:researchBuildPerformed = $true
    $script:researchInstallPerformed = $true
    $checkpointName = Get-PhoneLmCheckpointName `
        -Seed $Seed `
        -Layers $fixed.layers `
        -Tokens $fixed.tokens `
        -Dimension $Candidate.dimension `
        -FeedForwardDimension $Candidate.feed_forward_dimension `
        -Step $Steps
...
function Invoke-GenerationQuality($Candidate, [int]$Seed, [int]$Steps) {
    ...
    $tag = if ($Candidate.dimension -eq 32 -and $Candidate.feed_forward_dimension -eq 32) { '' } else { "-t$($fixed.tokens)-d$($Candidate.dimension)-f$($Candidate.feed_forward_dimension)" }
