...
- [QNN L19 probe-optimization audit (2026-08)](qnn-l19-probe-optimization-audit-2026-08/README.md) — CTX_CONCAT vs ATT_UPDATE の dev-token-exact 差（24/6 など）が表現の情報損失ではなく legacy Adam の座標標準化不足による最適化 artifact であることを、PCA-whitened + L2 + L-BFGS の canonical solver で検証し `C1_OPTIMIZATION_INSUFFICIENCY` と結論。
