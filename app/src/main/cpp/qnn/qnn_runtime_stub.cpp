...
bool Runtime::prepareTinyTransformerTraining(uint32_t,uint32_t,uint32_t,float,bool,std::string&e,uint32_t,TinyTransformerTrainingVariant,TinyTransformerTrainingTapSet,uint32_t numLayers,uint32_t numHeads,bool headwiseG1Gate){(void)numLayers;(void)numHeads;(void)headwiseG1Gate;e="QNN_DISABLED: tiny transformer training unavailable";return false;}
