...
bool Runtime::prepareMuonOptimizer(uint32_t,uint32_t,uint32_t,uint32_t,uint32_t,bool,std::string&e){e="QNN_DISABLED: HTP Muon optimizer unavailable";return false;}
bool Runtime::executeMuonOptimizer(const std::vector<float>&,const std::vector<float>&,const std::vector<float>&,const std::vector<float>&,const std::vector<float>&,const std::vector<float>&,const std::vector<float>&,const std::vector<float>&,float,MuonOptimizerOutputs&,std::string&e){e="QNN_DISABLED: HTP Muon optimizer unavailable";return false;}
bool Runtime::prepareMuonNsStageProbe(std::string&e){e="QNN_DISABLED: HTP Muon stage probe unavailable";return false;}
bool Runtime::executeMuonNsStageProbe(const std::vector<float>&,MuonNsStageOutputs&,std::string&e){e="QNN_DISABLED: HTP Muon stage probe unavailable";return false;}
...
bool Runtime::prepareTinyTransformerTraining(uint32_t,uint32_t,uint32_t,float,bool,std::string&e,uint32_t,TinyTransformerTrainingVariant,TinyTransformerTrainingTapSet,uint32_t numLayers,uint32_t numHeads,bool headwiseG1Gate,bool minimalGenerationOutputs){(void)numLayers;(void)numHeads;(void)headwiseG1Gate;(void)minimalGenerationOutputs;e="QNN_DISABLED: tiny transformer training unavailable";return false;}
