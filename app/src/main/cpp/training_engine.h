...
enum class ExecutionMode : int {
    ...
    // Nicopedia byte-level generation on the HTP graph: loads the private
    // NPRTCKPTV1 checkpoint written by the training milestone, runs the
    // device-side autoregressive loop (fixed prefix CPU/HTP parity, AR
    // parity, then greedy/sampled generation) and returns a private
    // KEY=VALUE report containing the generated bytes in hex.
    QNN_HTP_TINY_LANGUAGE_MODEL_NICOPEDIA_GENERATE = 101,
    // Nicopedia held-out evaluation on the HTP graph: loads a private
    // checkpoint (NPRTCKPTV1 or NPRTCKPTV2) and teacher-forces
    // validation.bin / development.bin through the HTP forward graph,
    // reporting NLL/perplexity/top-1/top-5/mean-rank/margin exactly like the
    // CPU host evaluator (htp_checkpoint_eval). No optimizer, no parity gate:
    // this is the HTP-native model-health metric.
    QNN_HTP_TINY_LANGUAGE_MODEL_NICOPEDIA_EVAL = 102,
};
