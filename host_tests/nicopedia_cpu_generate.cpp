...
// Loads an NPRTCKPTV1/V2 checkpoint and runs greedy or deterministic top-K
...
Checkpoint loadCheckpoint(const std::string& path) {
  ...
  if (v2) {
    // V2 appends Adam first/second moment registries. Generation does not
    // consume them, but fully validating and consuming both preserves the
    // same fail-closed identity/finiteness contract as the native reader.
    for (int moment = 0; moment < 2; ++moment) {
      ...
      for (std::uint32_t group = 0; group < momentCount; ++group) {
        ...
        input.read(reinterpret_cast<char*>(scratch.data()),
...
void writeRegistry(std::ostream& output,
                   const std::vector<phonelm::tiny_lm::ParameterInfo>& registry,
                   float fill) {
  ...
  for (const auto& entry : registry) {
    ...
    writeU64(output, entry.values->size());
    std::vector<float> values(entry.values->size(), fill);
    output.write(reinterpret_cast<const char*>(values.data()),
                 static_cast<std::streamsize>(values.size() * sizeof(float)));
  }
}

// Regression for the original CHECKPOINT_MAGIC failure: device training
// writes NPRTCKPTV2, while this host generator used to accept V1 only.
int checkpointV2SelfTest() {
  const std::string path = "nicopedia_cpu_generate_v2_self_test.ckpt";
  ...
  try {
    ...
    if (!output) throw std::runtime_error("SELF_TEST_OPEN_FAILED");
    ...
    writeU32(output, 7);
    writeU32(output, 3);
    writeRegistry(output, registry, 0.125f);
    writeRegistry(output, registry, 0.0f);
    writeRegistry(output, registry, 0.0f);
    output.close();
    if (!output) throw std::runtime_error("SELF_TEST_WRITE_FAILED");
    const Checkpoint loaded = loadCheckpoint(path);
    require(loaded.hasAdam, "V2 magic retained");
    require(loaded.seed == 7 && loaded.step == 3, "V2 seed/step");
    require(loaded.config.dimension == 4 &&
                loaded.config.feedForwardDimension == 8,
            "V2 dimensions");
    require(parametersFinite(loaded.parameters), "V2 parameter finite");
  } catch (...) {
    std::remove(path.c_str());
    throw;
  }
  std::remove(path.c_str());
  std::printf("nicopedia_cpu_generate_v2_self_test=PASS\n");
  return 0;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (argc == 2 && std::string(argv[1]) == "--checkpoint-v2-self-test")
      return checkpointV2SelfTest();
    if (argc < 5 || argc > 8) {
    ...
    std::cout << "NICOPEDIA_CPU_GENERATION\n"
              ...
              << "checkpoint_format="
              << (checkpoint.hasAdam ? "NPRTCKPTV2" : "NPRTCKPTV1") << "\n"
